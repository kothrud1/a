...
int main()
{
	...
	while(fgets(buffer,80,fp))// buffer stores the bytes it reads, 80 bytes read at a time from fp file
	{
		j = 0;
		strcpy(token1,"");
		strcpy(token2,"");
		strcpy(token3,"");
		strcpy(token4,"");
		tokencnt = sscanf(buffer,"%s%s%s%s",token1,token2,token3,token4);//sscanf reads from buffer, it reads string coz %s
		switch(tokencnt)
		{
